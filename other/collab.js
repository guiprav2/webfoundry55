...
class RealtimeCollaboration {
  constructor(room) {
    this.supabase = createClient(
      'https://xvxszwvhwzsxjhxskkek.supabase.co',
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh2eHN6d3Zod3pzeGpoeHNra2VrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTgyODY2MjcsImV4cCI6MjA3Mzg2MjYyN30.tQi5XePGi6NHLAfuKEuMSw7YQCiDVY2KAd8XXmk8Lcs',
    );
    this.channel = null;
  ...
  async joinRoom(room) {
    ...
    this.channel.on('presence', { event: 'sync' }, () => {
      ...
      this.presence = Object.keys(currentPresence).map(key => ({
        user: key,
        ...currentPresence[key][0],
      }));
      this.events.emit('presence:update', this.presence);
    });

    this.channel.on('presence', { event: 'join' }, ({ newPresences }) => {
      const joined = newPresences.map(presence => ({
        user: presence.user_key,
        ...presence,
      }));
    ...
    this.channel.on('presence', { event: 'leave' }, ({ leftPresences }) => {
      const left = leftPresences.map(presence => presence.user_key);
  ...
  async send(message) {
    if (this.channel) {
      await this.channel.send({
        type: 'broadcast',
        event: 'message',
        payload: { message: { ...message, peer: this.uid } },
      });
    }
  }
