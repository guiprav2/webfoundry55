...
s.rpc('shell:spawn', async ({ ws, shell, subdir, cols, rows }) => {
  ...
  let lastLabel;
  let procmonInternal = setInterval(async () => {
    try {
      ...
      ws.send(JSON.stringify({ type: 'shell', subtype: 'process', session, process }));
  ...
  term.on('exit', () => {
    clearInterval(procmonInternal);
    let term = terminals[session];
    delete terminals[session];
    ws.send(JSON.stringify({ type: 'shell:close', session, reason: term.killReason || 'exit' }));
  });
  return session;
});
